...
def init():
	...
	average_strat1 = [[(1/(grid_dim_x*grid_dim_y), 1/(grid_dim_x*grid_dim_y))] * grid_dim_y] * grid_dim_x
	global regret1
	regret1 = [[0] * grid_dim_y] * grid_dim_x
	global sigma2
	sigma2 = {} # Key: repr(routes), value: probability
	global regret2
	regret2 = {} # Key: repr(routes), value: regret
	...
	for _ in range(T):
		...
		for (grid_x, grid_y) in product(range(grid_dim_x), range(grid_dim_y)):
			cf_values1[grid_x][grid_y] = \
				cfr_player2([(base,base)], grid_x, grid_y, sigma1[grid_x][grid_y] * p1, p2,
				            route_length)
...
def cfr_player2(edge_history, grid_x, grid_y, p1, p2, rem_dist):
	...
	if rem_dist <= dist.iloc[curr_node, base] or (len(edge_history) > 1 and curr_node == base):
	...
	for edge_index, edge in enumerate(edges):
		...
		value, route = cfr_player2(edge_history.append(edge), grid_x, grid_y, p1,
						sigma2[] * p2, rem_dist - edge_data['distance'])
	...
	for edge_index, edge in enumerate(edges):
		regret2[routes[edge_index]] += p1 * (cf_values2[edge_index] - cf_value_curr)
		average_strat2[repr(routes[edge_index])] += p2 * sigma2[repr(routes[edge_index])]

	regret_matching2(routes)
...
def regret_matching2(routes):
	regret = {}
	for key, value in regret2:
		regret[key] = max(value, 0)
	den = sum(regret.values())
	if den > 0:
		for route in routes:
			sigma2[repr(route)] = regret[route] / den
	else:
		for route in routes:
			sigma2[repr(route)] = 1/len(routes)
